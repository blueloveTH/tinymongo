...
sub_cols = st.columns([1, 1, 1, 1, 2.5])

# add row
if sub_cols[0].button(tr.InsertRow):
    ...
    if len(selected_idx) > 0:
        ...
        df.loc[next_id, '?'] = True
# delete row
if sub_cols[1].button(tr.DeleteRow):
    save_draft_df()
...
if sub_cols[2].button(tr.ImportDB):
...
if sub_cols[3].button(tr.ExportDB):
    save_draft_df()
